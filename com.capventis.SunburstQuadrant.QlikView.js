...
// com.capventis.SunburstQuadrant.js - main functions to draw the chart 
...
///
function sbq_sector(_this, paper, cx, cy, r, rin, startAngle, endAngle, popupText, searchText, columnNumber, params) {
    ...
    if (columnNumber > -1) {
        arc.click(function () {
            _this.Data.SearchColumn(columnNumber, searchText, false);
        });
    }
    ...
    if (((endAngle - startAngle) > 2.5) && searchText.length > 0) 
    {
        ...
        txt.attr({ "font-size": (Math.round((rin - r) / 10)), "font-family": "Arial, Helvetica, sans-serif" });
        ...
        // the text object also needs to be clickable
        if (columnNumber > -1) {
            txt.click(function () {
                _this.Data.SearchColumn(columnNumber, searchText, false);
            });
...
function sbq_drawChart(divName) {

    // assign a local variable to store the extension object
    var _this = window[divName];

    // calculate the size of the canvas from the extension object
    var frameX = _this.GetWidth();
    var frameY = _this.GetHeight();

    // Count the levels of hierarchy
    var vColumnCount = _this.Data.Rows[0].length;
	...
	var sizeForData=Math.round(maxRad-segSize)-3;
    ...
    var level1colour = _this.Layout.Text1.text.toString(); 
    var level2colour = _this.Layout.Text2.text.toString();
    var level3colour = _this.Layout.Text3.text.toString();
    var level4colour = _this.Layout.Text4.text.toString();
    ...
    for (var i = 0; i < _this.Data.Rows.length; i++) {
        ...
        // The values are in _this.Data.Rows[i][vColumnCount - 1]
        var value = parseFloat(row[vColumnCount - 1].text);

        if (value == null) value = 0;

        // Level 1
        // These are stored in row[0]
        if (level1arr[row[0].text] == null)
        {
            level1arr[row[0].text] = value;
        }
        else
            level1arr[row[0].text] += value;
        overallTotal += value;
        ...
        if (vColumnCount > 2) {
            if (level2arr[row[0].text + '~~' + row[1].text] == null)
            {
                level2arr[row[0].text + '~~' + row[1].text] = value;
            }
            else
                level2arr[row[0].text + '~~' + row[1].text] += value;
        }
        ...
        if (vColumnCount > 3) {
            if (level3arr[row[0].text + '~~' + row[1].text + '~~' + row[2].text] == null)
            {
                level3arr[row[0].text + '~~' + row[1].text + '~~' + row[2].text] = value;            }
            else
                level3arr[row[0].text + '~~' + row[1].text + '~~' + row[2].text] += value;
        }
        ...
        if (vColumnCount > 4) {
            if (level4arr[row[0].text + '~~' + row[1].text + '~~' + row[2].text + '~~' + row[3].text] == null)
            {
                level4arr[row[0].text + '~~' + row[1].text + '~~' + row[2].text + '~~' + row[3].text] = value;
            }
            else
                level4arr[row[0].text + '~~' + row[1].text + '~~' + row[2].text + '~~' + row[3].text] += value;
        }
    ...
    var linecolor = _this.Layout.Text0.text.toString(); //"#F0F0FA";  // default to light grey for now
    ...
    sbq_sector(_this, paper, 0, frameY, level0.start, level0.end, 0, 90, "", "", -1, { fill: "#fee501", stroke: linecolor, "stroke-width": "1px" });

    // central text for level 0
    var txt = paper.text(segSize / 2, frameY - (segSize / 3), 'Total :\n' + sbq_addCommas(overallTotal.toFixed(2)));
    txt.attr({ "font-size": (Math.round(segSize / 10)), "font-family": "Arial, Helvetica, sans-serif" });
    txt.click(function () {
        _this.Data.ClearSelections();
    });
    ...
    // Draw level 1
    sbq_drawLevel(0, level1arr, level1colour, divName, paper, frameY, level1.start, level1.end, level1min, level1max, overallTotal, linecolor);

    // Draw level 2
    if (vColumnCount > 2)
        sbq_drawLevel(1, level2arr, level2colour, divName, paper, frameY, level2.start, level2.end, level2min, level2max, overallTotal, linecolor);

    // Draw level 3
    if (vColumnCount > 3)
        sbq_drawLevel(2, level3arr, level3colour, divName, paper, frameY, level3.start, level3.end, level3min, level3max, overallTotal, linecolor);

    // Draw level 4
    if (vColumnCount > 4)
        sbq_drawLevel(3, level4arr, level4colour, divName, paper, frameY, level4.start, level4.end, level4min, level4max, overallTotal, linecolor);
...
///
function sbq_drawLevel(columnNumber, level_arr, level_colour, divName, paper, frameY, start_radius, end_radius, level_min, level_max, overallTotal, linecolor) {
    var _this = window[divName];
    var startAngle = 0;
    ...
    for (var o in level_arr) {
        ...
        var vColor = sbq_colormix(((level_arr[o] - level_min) / (level_max - level_min)), '#ffffff', level_colour);
        //var vColor = level_colour[o];

        // QlikView Num(color, '(hex)') will return 8 chars
        var vOpacity='ff';
        if(vColor.length==8)
        {
            vOpacity=vColor.substring(0,2);
            vColor=vColor.substring(2,8);
        }
        vOpacity=sbq_h2d(vOpacity)/255;

        // It also needs to have the #
        if(vColor.substring(0,1)!='#') vColor='#' + vColor;

        // call the sector function to draw the segment
        sbq_sector(_this, paper, 0, frameY, start_radius, end_radius, startAngle, startAngle + angle, vTable, vName, columnNumber, { fill: vColor, stroke: linecolor, "stroke-width": "1px", "opacity" : vOpacity });
